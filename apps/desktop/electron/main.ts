...
ipcMain.handle("cache:check", async (event, trackId: number, originalPath: string) => {
...
ipcMain.handle("cache:download", async (event, trackId: number, url: string, token?: string) => {
...
protocol.registerSchemesAsPrivileged([
  ...
  },
  {
    ...
    privileges: {
      ...
      secure: true,
      supportFetchAPI: true,
      bypassCSP: true, // Allow media loading
      stream: true
    }
  }
])
...
app.whenReady().then(() => {
  ...
  protocol.handle('media', (request) => {
    const url = new URL(request.url);
    // Remove the leading slash and avoid trailing slashes
